...
> ### Availability note (please read first)
>
> **This analysis ships in the [jjstatsplot module](https://www.serdarbalci.com/jjstatsplot/),
> not in ClinicoPathDescriptives.**
>
> `hullplot` is a released analysis of `jjstatsplot`
> (menu: **Analyses → JJStatsPlot → Continuous vs Continuous → Hull Plot**), and its five
> example datasets (`hullplot_customer_data`, `hullplot_clinical_data`,
> `hullplot_experimental_data`, `hullplot_survey_data`, `hullplot_quality_data`) ship with
> that package too. Nothing in this vignette is provided by
> `ClinicoPathDescriptives` 1.0.52, whose 14 analyses are `agepyramid`, `alluvial`,
> `benford`, `categorize`, `checkdata`, `chisqposttest`, `crosstable`, `dataquality`,
> `outlierdetection`, `reportcat`, `summarydata`, `tableone`, `vartree` and `venn`.
>
> To run the examples, install and load `jjstatsplot` and call `jjstatsplot::hullplot()`
> with `package = "jjstatsplot"` when loading the data. The chunks below have been updated
> accordingly, but are kept at **`eval = FALSE`** because `jjstatsplot` is not a dependency
> of this package; the output shown is illustrative only.
...
data(hullplot_customer_data, package = "jjstatsplot")
data(hullplot_clinical_data, package = "jjstatsplot")
data(hullplot_experimental_data, package = "jjstatsplot")
data(hullplot_survey_data, package = "jjstatsplot")
data(hullplot_quality_data, package = "jjstatsplot")
