...
> ### Module note --- `contTablesPaired` is not in ClinicoPathDescriptives
>
> **This analysis ships in the umbrella ClinicoPath module**
> (<https://www.serdarbalci.com/ClinicoPathJamoviModule/>), **not in
> ClinicoPathDescriptives 1.0.52.** `contTablesPaired` is defined and exported there;
> calling it after `library(ClinicoPathDescriptives)` will fail with
> *"could not find function"*.
>
> In the umbrella build it is routed to a development menu group (`ExplorationD`), so it
> may not be visible in a released jamovi menu. jamovi's own built-in **jmv** module
> already provides the equivalent McNemar analysis under
> **Frequencies -> Paired Samples**.
>
> The `prostate_agreement_data` dataset used below also ships with the umbrella
> ClinicoPath package, not with ClinicoPathDescriptives.
>
...
> None of them performs McNemar's test; for unpaired categorical association in this
> module see **`crosstable`** or **`chisqposttest`**.
>
...
# NOTE: contTablesPaired lives in the umbrella ClinicoPath package, NOT in
