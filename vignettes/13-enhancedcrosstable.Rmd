...
>
> **`enhancedcrosstable` is not shipped by ClinicoPathDescriptives.** This
> module provides 14 analyses (`agepyramid`, `alluvial`, `benford`,
> `categorize`, `checkdata`, `chisqposttest`, `crosstable`, `dataquality`,
> `outlierdetection`, `reportcat`, `summarydata`, `tableone`, `vartree`,
> `venn`); `enhancedcrosstable` is not among them.
>
> The document below is a **design note for a planned future release** and is
> kept for reference. None of its `enhancedcrosstable()` code is executed, and
> the option names, defaults and levels it shows are provisional - they have not
> been fixed against a shipped analysis definition and should not be relied on.
>
> For cross-tabulation available **today**, use `crosstable()` (see
> *Generating a Publication-Ready 'Table 1' with crosstable*) or `tableone()`.
> The section *Comparison with Standard Cross Tables* below documents the real
> `crosstable()` options.
...
```r
# Standard ClinicoPath crosstable (shipped today).
# Options: vars, group, sty, excl, cont, pcat, p_adjust, showSMD.
crosstable(
  data = histopathology,
  ...
  p_adjust = "none",   # none | bonferroni | holm | BH | BY
  excl = FALSE,
  showSMD = FALSE
)

# Enhanced crosstable with danchaltiel integration (planned, not yet shipped)
enhancedcrosstable(
  data = histopathology,
  vars = c("Grade_Level", "LVI", "PNI"),
  by_var = "Group",
...
| Feature | Standard crosstable (shipped) | Enhanced crosstable (planned) |
|---------|-------------------|-------------------|
| **Variable Selection** | Multiple variables via `vars`, one grouping variable via `group` | Multiple variables |
| **Statistical Tests** | `pcat`: `chisq` or `fisher` | Automated selection |
| **Multiplicity** | `p_adjust`: `none`, `bonferroni`, `holm`, `BH`, `BY` | Automated |
| **Effect Sizes** | Standardized mean differences via `showSMD` | Automated calculation |
| **Output Styles** | `sty`: `arsenal`, `finalfit`, `gtsummary`, `nejm`, `lancet`, `hmisc` | HTML, Word, CSV |
| **Missing Data** | `excl` (exclude incomplete cases) | Comprehensive options |
| **Tidyselect Syntax** | Not supported | Full support |
| **Formula Interface** | Not supported | Advanced transformations |

## When to Use Each Approach

### **Standard crosstable** (available now)
- Multi-variable "Table 1" for a single grouping variable
- Journal-specific layouts through `sty`
- Chi-square or Fisher tests with optional multiplicity adjustment
- Group-balance checking through `showSMD`

### **Enhanced crosstable** (planned for a future release)
- Automated per-variable test selection
- Comprehensive effect sizes with confidence intervals
- Word/CSV export paths
- Tidyselect and formula-based variable transformation
...
- **Function Help**: `?crosstable` for the parameters available today
  (`?enhancedcrosstable` will exist only once the analysis ships)
...
The enhanced cross-table functionality is **planned for a future release** and is
not part of ClinicoPathDescriptives today. By integrating the sophisticated
danchaltiel/crosstable package with ClinicoPath's clinical focus, it is intended
to offer:
