...
>
> The analysis documented here is `jcorrelation` ("Correlation Analysis"). It currently
> exists only in the development umbrella module `ClinicoPath`, where it is still routed to
> a development menu group (`ExplorationD`) rather than a released one. It is not shipped by
> `ClinicoPathDescriptives`, nor by `jjstatsplot`, `meddecide`, `jsurvival` or `OncoPath`.
...
> and `venn`.
>
> If you need a correlation matrix today, use **Correlation Matrix** (`jjcorrmat`) in the
> [jjstatsplot module](https://www.serdarbalci.com/jjstatsplot/).
>
> Because the function is not installed, **every code chunk in this vignette is set to
> `eval = FALSE`** and the output shown is illustrative only. The prose below also refers to
> the function as `correlation`; the actual analysis name is `jcorrelation`.
...
comprehensive_analysis <- jcorrelation(
  data = histopathology,
  vars = c(
    "Age", "Grade", "TStage", "Anti-X-intensity", "Anti-Y-intensity",
