...
`agepyramid()` has no grouping option, so a stratified pyramid is produced by
subsetting the data and calling the analysis once per stratum. `female` and
`male` are `Level` options and therefore **required** arguments of the R
wrapper.
...
### Age Pyramid for the Treatment Arm

```{r}
# Pyramid restricted to the treatment arm
agepyramid(
  data = dplyr::filter(viz_data, Group == "Treatment"),
  age = "Age",
  gender = "Sex",
  female = "Female",
  male = "Male",
  plot_title = "Age Pyramid - Treatment Arm"
)
```

### Age Pyramid for High-Risk Patients

```{r}
# Pyramid restricted to the high-risk stratum.
# `age_groups` accepts custom, pediatric, reproductive, geriatric or lifecourse;
# `bin_width` (default 5) applies to the "custom" scheme.
agepyramid(
  data = dplyr::filter(viz_data, Risk_Profile == "High Risk"),
  age = "Age",
  gender = "Sex",
  female = "Female",
  male = "Male",
  ...
  plot_title = "Age Pyramid - High Risk"
)
```
...
`venn()` only accepts **factor** variables, and the "true" level is named as a
character string. `var1true` ... `var7true` are `Level` options, so all seven are
required arguments of the R wrapper - pass `NULL` for unused slots.

```{r}
# Prepare categorical indicators for Venn analysis (factors, not 0/1 numerics)
venn_data <- viz_data %>%
  mutate(
    High_Grade = factor(ifelse(Grade == "3", "Yes", "No"))
  )
...
```{r eval=FALSE}
# Comprehensive variable tree (disabled due to technical issues)
vartree(
  data = viz_data,
...
> **Not part of this module.** `waterfall()` and `swimmerplot()` are **not**
> shipped by ClinicoPathDescriptives - they are provided by the companion
> **OncoPath** module (jamovi menu: `Exploration > Patient Follow-Up Plots`).
> The examples in Sections 5 and 6 are kept for reference and are set to
> `eval = FALSE`; run them after installing OncoPath.

### Waterfall Plot - Individual Response

```{r eval=FALSE}
# Requires the OncoPath module - not available in ClinicoPathDescriptives
...
# `benford` takes exactly one variable (`var`); `digits` (default 2) selects
# first-digit vs first-two-digit analysis.
benford(
  data = viz_data,
  var = "MeasurementA",
  digits = 2
...
### Benford Analysis of a Second Variable

`benford()` analyses one variable per call, so a second variable means a second
call.
...
### Navigation Path (ClinicoPathDescriptives)
...
### Navigation Path (other modules)
- **Treatment Response**: `Exploration > Patient Follow-Up Plots > Treatment Response Analysis` (**OncoPath**)
- **Timelines**: `Exploration > Patient Follow-Up Plots > Swimmer Plot` (**OncoPath**)
...
This visualization gallery demonstrates the visualization capabilities of ClinicoPathDescriptives - age pyramids, alluvial diagrams, variable trees, Venn/UpSet plots and Benford analysis - together with the treatment-response plots contributed by the companion OncoPath module (Sections 5 and 6), providing templates and examples for creating impactful, publication-ready figures that effectively communicate research findings.
