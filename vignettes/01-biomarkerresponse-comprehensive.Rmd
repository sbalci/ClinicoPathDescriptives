...
> ## Not part of ClinicoPathDescriptives 1.0.52
>
> **`biomarkerresponse()` — Biomarker Response Association — is under development and is not
> shipped by ClinicoPathDescriptives 1.0.52.** It exists only in the umbrella
> [ClinicoPath](https://github.com/sbalci/ClinicoPathJamoviModule) development tree, where it
> currently carries a development/test menu group, so it does not appear in the jamovi menu of
> any released module and the R wrapper is not exported. Its intended home on release is
> [OncoPath](https://www.serdarbalci.com/OncoPath/), which does not ship it yet either.
>
> Option names, defaults and level names shown below are provisional and may change before it
> ships. Every code chunk in this vignette is therefore set to `eval = FALSE`.
>
...
# NOTE: biomarkerresponse() is NOT exported by ClinicoPathDescriptives 1.0.52.
# Only the bundled datasets below come from this package.
