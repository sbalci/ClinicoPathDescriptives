...
# The ggprism analysis is not installed with this module, so nothing here can run.
...
>
> `ggprism` currently exists only in the development umbrella module `ClinicoPath`, where it
> is still routed to a development menu group (`JJStatsPlotD`) rather than a released one.
> It is not shipped by `ClinicoPathDescriptives`, and it is not yet in the released
> [jjstatsplot module](https://www.serdarbalci.com/jjstatsplot/) either.
...
> and `venn`.
>
> The example datasets used below (`clinical_prism_data`, `dose_response_prism_data`, ...)
