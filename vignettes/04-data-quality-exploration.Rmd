...
The same assessment is available as a shipped analysis. `dataquality()` covers a set of
variables at once, and `checkdata()` drills into a single variable:

```{r}
# Multi-variable visual quality overview
dataquality(
  data = histopathology,
  vars = c("Age", "Sex", "Grade", "TStage", "MeasurementA", "MeasurementB"),
  check_duplicates = TRUE,           # default FALSE
  check_missing = TRUE,              # default FALSE
  complete_cases_only = FALSE,       # default FALSE
  plot_data_overview = TRUE,         # default FALSE
  plot_missing_patterns = TRUE,      # default FALSE
  plot_data_types = FALSE,           # default FALSE
  missing_threshold_visual = 10,     # default 10 (%)
  showSummary = TRUE,                # default TRUE
  showRecommendations = TRUE,        # default TRUE
  showExplanations = TRUE            # default FALSE
)
...
checkdata(
  data = histopathology,
  var = "Age",
  showOutliers = TRUE,               # default TRUE
  showDistribution = TRUE,           # default FALSE
  showDuplicates = TRUE,             # default FALSE
  showPatterns = FALSE,              # default FALSE
  rareCategoryThreshold = 5,         # default 5 (%)
  clinicalValidation = TRUE,         # default TRUE
  unitSystem = "auto",               # auto | metric | imperial
  outlierTransform = "none",         # none | log | sqrt
  mcarTest = FALSE,                  # default FALSE
  cvMinMean = 0.01                   # default 0.01
)
```
...
if(length(available_vars) > 0) {
  tryCatch({
    summarydata(
      data = histopathology,
      vars = available_vars,
      distr = TRUE,        # normality / distribution diagnostics (default FALSE)
...
# benford() takes only `var` and `digits` (Integer, default 2, range 1-3):
# benford(data = financial_data, var = "FinancialAmount", digits = 2)
...
# Note: Benford analysis would be performed here
# benford(data = expense_data, var = "ExpenseAmount", digits = 2)
...
# benford(data = histopathology, var = "OverallTime", digits = 2)
...
### Using the shipped `outlierdetection` analysis

```{r}
outlierdetection(
  data = histopathology,
  vars = c("Age", "OverallTime", "MeasurementA", "MeasurementB"),
  method_category = "composite",           # univariate | multivariate | composite | all
  univariate_methods = "zscore_robust",    # zscore_robust | zscore | iqr | eti | hdi
  multivariate_methods = "mahalanobis",    # mahalanobis | mahalanobis_robust | mcd | optics | lof
  composite_threshold = 0.5,               # default 0.5
  zscore_threshold = 3.29,                 # default 3.29
  iqr_multiplier = 1.7,                    # default 1.7
  confidence_level = 0.999,                # default 0.999
  show_outlier_table = TRUE,               # default TRUE
  show_method_comparison = TRUE,           # default FALSE
  show_exclusion_summary = TRUE,           # default FALSE
  show_visualization = TRUE,               # default FALSE
  show_interpretation = TRUE,              # default FALSE
  sampleThreshold = 10000,                 # default 10000
  sampleSize = 5000,                       # default 5000
  seed = 123                               # default 123
)
```

#### Large datasets are subsampled — and that costs you outliers

Above `sampleThreshold` rows (default `10000`), the analysis draws a random
subsample of `sampleSize` rows (default `5000`) and works on that. This matters
more here than for most statistics: a random subset can only reveal the outliers
it happens to *contain*, so the reported count is a **lower bound**, not a
complete list. On a 12,000-row dataset with four planted gross outliers, only the
one that fell in the subsample could be found.

The analysis says so in its own panel, reports the original N alongside the
subsample, and names the proportion retained. If you are screening for individual
erroneous values to verify against source records — rather than looking for a
systematic pattern — raise `sampleThreshold` above your row count so the whole
dataset is analysed:

```{r outlier-full-dataset, eval=FALSE}
outlierdetection(
  data            = big_cohort,
  vars            = c("Age", "MeasurementA"),
  sampleThreshold = 100000    # above nrow(big_cohort): analyse every row
)
```

Raising `sampleSize` instead keeps more of the data without disabling subsampling
altogether. Both are slower; `seed` makes whichever subsample you get
reproducible.

### Statistical Outlier Analysis (manual, for comparison)
...
1. **Benford's Law**: `Exploration > ClinicoPath Data Quality > Benford Analysis`
2. **Single Variable Quality Check**: `Exploration > ClinicoPath Data Quality > Single Variable Quality Check`
3. **Multi-Variable Visual Quality**: `Exploration > ClinicoPath Data Quality > Multi-Variable Visual Quality`
4. **Outlier Detection**: `Exploration > ClinicoPath Data Quality > Outlier Detection`
5. **Variable Summary**: `Exploration > ClinicoPath Descriptives > Summary of Continuous Variables` / `Summary of Categorical Variables`
6. **Cross-tabulations**: `Exploration > ClinicoPath Comparisons > Cross Tables`
7. **Variable Tree**: `Exploration > ClinicoPath Descriptive Plots > Variable Tree`
