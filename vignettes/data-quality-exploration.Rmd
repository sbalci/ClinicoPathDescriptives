...
summarydata(
  ...
  title = "Distribution Assessment - Continuous Variables"
)
```
...
reportcat(
  ...
  title = "Distribution Assessment - Categorical Variables"
)
```
...
vartree(
  data = histopathology,
  vars = c("Group", "Grade", "TStage", "LVI", "PNI", "LymphNodeMetastasis", "Outcome"),
  title = "Variable Relationship Structure"
)
```
...
if(length(date_vars) > 0) {
  ...
  reportcat(
    ...
    title = "Date Variable Distribution"
  )
}
```
...
crosstable(
  ...
  total = TRUE,
  percentages = "both",
  title = "Death Status vs 5-Year Mortality Consistency Check"
)
```
...
crosstable(
  ...
  total = TRUE,
  percentages = "both",
  title = "Primary Outcome vs Death Status Consistency"
)
```
...
crosstable(
  ...
  total = TRUE,
  percentages = "row",
  title = "Tumor Grade vs T-Stage Distribution"
)
```
...
benford(
  data = histopathology, 
  var = "MeasurementB",
  title = "Benford's Law Analysis - Measurement B"
)
```
...
benford(
  data = histopathology,
  var = "OverallTime", 
  title = "Benford's Law Analysis - Follow-up Time"
)
```
...
if(length(rater_vars) > 0) {
  ...
  if(length(binary_raters) > 0) {
    reportcat(
      ...
      title = "Inter-rater Reliability - Binary Ratings"
    )
  }
  ...
  if(length(categorical_raters) > 0) {
    reportcat(
      ...
      title = "Inter-rater Reliability - Categorical Ratings"
    )
  }
}
```
...
if(length(test_vars) >= 2) {
  ...
  crosstable(
    ...
    total = TRUE, 
    percentages = "both",
    title = "Diagnostic Test Performance vs Gold Standard"
  )
}
```
...
if(length(validity_vars) > 0) {
  reportcat(
    ...
    title = "Data Validity Assessment"
  )
}
```
