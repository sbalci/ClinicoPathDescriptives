...
# The missingdata analysis is not installed with this module, so nothing here can run.
...
>
> `missingdata` currently exists only in the development umbrella module `ClinicoPath`,
> where it is still routed to a development menu group (`ExplorationD`) rather than a
> released one. It is not shipped by `ClinicoPathDescriptives`, nor by `jjstatsplot`,
...
> and `venn`. For missing-data work available today in this module, see
> **Single Variable Quality Check** (`checkdata`, which offers `mcarTest`) and
> **Multi-Variable Visual Quality** (`dataquality`, which offers `check_missing`,
> `plot_missing_patterns` and `complete_cases_only`). Neither performs imputation.
>
> The example datasets used below (`clinical_trial_data`, `biomarker_data`, ...) are not
> shipped by `ClinicoPathDescriptives`. Because neither the function nor the data is
> available here, **every code chunk in this vignette is set to `eval = FALSE`** and the
> output shown is illustrative only.
