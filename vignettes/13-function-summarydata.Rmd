...
brca_data <- data.frame(
  ...
  TreatmentGroup = sample(c("Control", "Treatment"), n_patients, replace = TRUE)
)
...
# summarydata() takes: vars, distr, decimal_places, outliers, report_sentences.
# It has no grouping option - use tableone() or crosstable() to compare groups.
results <- summarydata(
    data = brca_data,
...
results$outlierReport   # (shown when outliers = TRUE)
results$reportSentences # (shown when report_sentences = TRUE)
```
