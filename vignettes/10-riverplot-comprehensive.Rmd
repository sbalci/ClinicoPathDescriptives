---
title: "riverplot: Comprehensive River Plot and Flow Visualization"
author: "ClinicoPath"
date: "`r Sys.Date()`"
...
> ## In development
>
> **`riverplot()` — River Plots & Alluvial Diagrams — is still in development and is not
> yet in the released ClinicoPathDescriptives module.** It carries a development menu
> group, so it does not appear in the jamovi menu in a released build and the R wrapper is
> not exported.
>
> `riverplot` exists only in the umbrella
> [ClinicoPath](https://github.com/sbalci/ClinicoPathJamoviModule) development tree. No
> released module ships it: **not jjstatsplot** (which was where this vignette came from,
> written against the earlier name `jjriverplot`), and not meddecide, jsurvival or OncoPath.
> Option names and defaults may still change before it ships.
>
> The released equivalent in ClinicoPathDescriptives 1.0.52 today is **`alluvial`** (Alluvial
> Diagrams), which draws the same flow diagrams via `easyalluvial`/`ggalluvial`.
>
> Every code chunk below is `eval = FALSE`.

```{r vignette-eval-guard, include = FALSE}
# This vignette documents an unreleased analysis; nothing here is executed.
knitr::opts_chunk$set(eval = FALSE)
```

## Overview

The `riverplot` function provides a comprehensive interface for creating river plots (alluvial diagrams), Sankey diagrams, and stream graphs to visualize flows, transitions, and pathways over time or between categories. This function is built on `ggplot2` and `ggalluvial`, offering multiple visualization styles with performance optimization through internal caching.
...
# NOTE: riverplot() is not exported by any released module. jjstatsplot does NOT
# provide it. Once released it will be reachable from the ClinicoPath umbrella module.
library(ClinicoPath)
library(ggplot2)
```
...
data(riverplot_test_data_wide)

# Multi-stage treatment pathway
result_pathway <- riverplot(
  data = riverplot_test_data_wide,
...
treatment_comparison <- riverplot(
  data = riverplot_test_data_wide,
  strata = c("month3_response", "month6_response", "month12_response"),
  plotType = "sankey",
  fillType = "first",
...
control_data <- riverplot_test_data_wide[
  riverplot_test_data_wide$treatment_arm == "Control", ]

control_pathway <- riverplot(
  data = control_data,
  strata = c("month3_response", "month6_response", "month12_response"),
  plotType = "sankey",
...
# Load education data
data(riverplot_education_data)

# Student performance progression
education_flow <- riverplot(
  data = riverplot_education_data,
...
# Load marketing data  
data(riverplot_marketing_data)

# Customer journey through marketing funnel
marketing_funnel <- riverplot(
  data = riverplot_marketing_data,
...
site_subset <- riverplot_test_data_long[
  riverplot_test_data_long$hospital_center %in% high_performing_sites, ]

site_comparison <- riverplot(
  data = site_subset,
  time = "timepoint",
  strata = "treatment_response",
  weight = "treatment_cost",
...
high_cost_patients <- riverplot_test_data_long[
  riverplot_test_data_long$treatment_cost > 5000, ]

high_cost_analysis <- riverplot(
...
elderly_patients <- riverplot_test_data_long[
  riverplot_test_data_long$age_group == "75+", ]

elderly_analysis <- riverplot(
...
for (treatment in treatment_groups) {
  subset_data <- riverplot_test_data_long[
    riverplot_test_data_long$treatment_arm == treatment, ]
  
  plot_result <- riverplot(
    data = subset_data,
    time = "timepoint",
    strata = "treatment_response", 
    plotType = "alluvial",
...
center_comparison <- function(center_id) {
  center_data <- riverplot_test_data_long[
    riverplot_test_data_long$hospital_center == center_id, ]
...
# Analyze each center
centers <- unique(riverplot_test_data_long$hospital_center)
...
The `riverplot` function is built on:
...
The optimized `riverplot` function provides:
