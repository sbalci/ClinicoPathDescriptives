...
>
> **`autoeda()` — Automated Exploratory Data Analysis — is under development and is not shipped
> by ClinicoPathDescriptives 1.0.52.** It exists only in the umbrella
> [ClinicoPath](https://github.com/sbalci/ClinicoPathJamoviModule) development tree, where it
> currently carries a development menu group, so it does not appear in the jamovi menu of a
> released build and the R wrapper is not exported. No sibling module (jjstatsplot, meddecide,
> jsurvival, OncoPath) ships it either.
>
...
>
> For released descriptive/data-screening analyses in this module today, see `dataquality`,
> `checkdata`, `summarydata`, `reportcat` and `outlierdetection`.
...
# NOTE: autoeda() is NOT exported by ClinicoPathDescriptives 1.0.52.
# Only the bundled histopathology dataset below comes from this package.
...
autoeda(
  data = mydata,
  vars = c(
    "Age", "Sex", "Grade", "TStage", "OverallTime",
...
autoeda(
  data = histopathology,
  vars = c(
    "Age", "Sex", "Race", "Grade", "TStage", "NStage",
