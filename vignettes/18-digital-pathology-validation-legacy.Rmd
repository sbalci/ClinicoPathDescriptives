...
>
> **This vignette does not document any ClinicoPathDescriptives analysis.**
>
> It is a general statistical-methods tutorial on digital pathology validation. Every
> computation below is plain R (`cor`, `lm`, `t.test`, `dplyr`) plus `psych::ICC()`; it does
> not call any jamovi analysis from ClinicoPathDescriptives, jjstatsplot, meddecide,
> jsurvival or OncoPath. The `digital_pathology_validation` dataset it loads is **not**
> installed by ClinicoPathDescriptives, and `psych` / `broom` are not dependencies of this
> package, so all chunks are non-evaluating (`eval = FALSE` is already set globally below).
>
> This file is retained as a "legacy" methodological reference only. The closest released
> tooling for the agreement analyses described here is `agreement` in
> [meddecide](https://www.serdarbalci.com/meddecide/) and `ihcheterogeneity` in
> [OncoPath](https://www.serdarbalci.com/OncoPath/) — neither of which is exercised in this
> vignette.
>
...
cat(
  "Dataset dimensions:", nrow(digital_pathology_validation), "cases ×",
  ncol(digital_pathology_validation), "variables\n"
)

# Key variables summary
summary(digital_pathology_validation[, c(
  "Ki67_Manual", "Ki67_AI_Assisted",
