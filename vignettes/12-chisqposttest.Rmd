...
Two separate options control residuals output:

- `plot` (default `FALSE`) draws the standardized residual heatmap.
- `showResiduals` (default `FALSE`) adds the residuals analysis tables.
- `residualsCutoff` (default `2`) sets the |residual| threshold flagged as a
  notable contribution to the chi-square statistic.
...
chisqposttest(
  ...
  excl = TRUE,
  exp = FALSE,
  plot = TRUE,            # Generate residuals plot
  showResiduals = TRUE,   # Add residuals analysis tables
  residualsCutoff = 2
)
```
...
When the residual plot is enabled:
...
chisqposttest(
  ...
  excl = TRUE,
  exp = TRUE,
  plot = FALSE   # set TRUE for the residual heatmap
)
```
