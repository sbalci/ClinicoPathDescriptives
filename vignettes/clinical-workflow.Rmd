...
The `vartree` function from `ClinicoPathDescriptives` provides a visual overview of the data structure and relationships between variables. Due to the complexity of rendering interactive `vartree` outputs directly within this static vignette, the code below demonstrates its usage. To view the full interactive output, please run this code locally in your R environment.

Note that `vartree` has no defaults for `vars`, `percvar`, `summaryvar`, `prunebelow` and `follow`, nor for the `Level` options `percvarLevel`, `pruneLevel1`, `pruneLevel2`, `followLevel1` and `followLevel2` - the jamovi compiler forbids defaults on `Level`. All of them are therefore required arguments of the R wrapper and must be passed explicitly, using `NULL` where the feature is unused.

```{r}
# Generate variable tree for data structure overview
# Check if vartree function is available and works properly
if(requireNamespace("ClinicoPathDescriptives", quietly = TRUE)) {
  tryCatch({
    results <- vartree(
      data = clinical_data,
  ...
  cat("ClinicoPathDescriptives package not available for variable tree\n")
...
# Level, so both are REQUIRED arguments of the R wrapper.
# `agepyramid` has no grouping option - subset the data for a per-group pyramid.
agepyramid(
  data = clinical_data,
  age = "Age",
  gender = "Sex",
...
# `summarydata` accepts only continuous `vars`. It has no grouping option, so
# stratified summaries are obtained by subsetting the data before the call.
...
### Biomarker Distribution Within a Tumour Grade

```{r}
# Biomarker analysis for grade 3 tumours
summarydata(
  data = dplyr::filter(clinical_data, Grade == 3),
...
# `crosstable` requires `group` to be categorical (permitted: factor), so use the
# recoded Outcome_Status / Survival_Status rather than the numeric `Outcome`.
outcome_analysis <- crosstable(
  data = clinical_data,
  vars = c("Outcome_Status", "Survival_Status"),
  group = "Group",
  sty = "nejm",
  pcat = "chisq"
)
...
# `p_adjust` controls multiple-testing correction:
# none (default), bonferroni, holm, BH or BY.
crosstable(
  data = clinical_data,
  vars = c("Grade", "TStage", "LymphNodeMetastasis"),
...
```{r}
# Venn diagram of invasion markers and outcome.
# `venn` only accepts FACTOR variables (permitted: factor), so keep the original
# factors instead of recoding them to 0/1 numerics.
# `var1true` ... `var7true` are Level options and therefore REQUIRED arguments of
# the R wrapper: pass the positive level, or NULL for an unused variable slot.
clinical_venn <- clinical_data %>%
  mutate(
    Poor_outcome = factor(ifelse(Outcome_Status == "Event Occurred", "Yes", "No"))
  )
...
# `benford` takes a single continuous variable; `digits` (default 2, range 1-3)
# selects first-digit vs first-two-digit analysis.
benford(
  data = clinical_data,
  var = "MeasurementA",
  digits = 2
...
### Distribution of Rater Scores

Note: ClinicoPathDescriptives does not compute inter-rater agreement
statistics - `agreement()` and `icccoeff()` belong to the **meddecide** module.
`reportcat()` below reports the frequency distribution of each rater's
categorical scores only.

```{r}
# Frequency summary of each rater's scores
