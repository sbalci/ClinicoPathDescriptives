...
>
> **`eurostatmap` is NOT part of ClinicoPathDescriptives 1.0.52.**
>
...
>
> **It is under development and planned for a future release.** Until then `eurostatmap()`
> cannot be called from this package. The `eurostat_*` and `turkey_*` example datasets used
> below are also not installed by ClinicoPathDescriptives, so every code chunk is set to
> `eval = FALSE`.
>
...
```{r global-eval-off, include=FALSE}
