...
> methodological walkthrough written in plain R (`dplyr`, `pROC`, `survival`) rather than a
> guide to any of the 14 analyses this module ships (`agepyramid`, `alluvial`, `benford`,
> `categorize`, `checkdata`, `chisqposttest`, `crosstable`, `dataquality`, `outlierdetection`,
> `reportcat`, `summarydata`, `tableone`, `vartree`, `venn`).
>
> The jamovi analyses that cover this material live in other modules:
> survival and Cox modelling in [jsurvival](https://www.serdarbalci.com/jsurvival/),
> ROC and diagnostic-test evaluation in [meddecide](https://www.serdarbalci.com/meddecide/),
> and IHC/oncology plots in [OncoPath](https://www.serdarbalci.com/OncoPath/).
>
> The datasets used below (`ihc_molecular_comprehensive`, `digital_pathology_validation`,
> `biomarker_validation_study`, `precision_oncology_data`) are **not** shipped with
