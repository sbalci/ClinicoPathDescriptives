...
>
> **`clinmon` is NOT part of ClinicoPathDescriptives 1.0.52.**
>
> This analysis currently exists only in the `ClinicoPath` umbrella development tree, where
> it is routed to a development-only menu group (`ClinicoPathD`). It is not shipped by
...
>
> **It is under development and planned for a future release.** Until then `clinmon()`
> cannot be called from this package, so every code chunk below is set to `eval = FALSE` and
> the options documented here describe the in-development version, not a released API.
>
