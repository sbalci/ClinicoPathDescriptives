...
>
> **`desctools` ("Advanced Statistical Tests") is NOT part of ClinicoPathDescriptives 1.0.52.**
>
> This analysis currently exists only in the `ClinicoPath` umbrella development tree, where
> it is routed to a development-only menu group (`ExplorationD`). It is not shipped by
> ClinicoPathDescriptives, and it is not shipped by any sibling module either
> ([jjstatsplot](https://www.serdarbalci.com/jjstatsplot/),
> [meddecide](https://www.serdarbalci.com/meddecide/),
> [jsurvival](https://www.serdarbalci.com/jsurvival/), or OncoPath).
>
> **It is under development and planned for a future release.** Until then `desctools()`
> cannot be called from this package. All chunks in this vignette are already non-evaluating
> (`eval = FALSE` is set globally below), and the options documented here describe the
> in-development version, not a released API.
>
...
> For categorical association testing that IS available today, see `chisqposttest` and
> `crosstable`.
