...
vignette: >
  %\VignetteIndexEntry{Comprehensive Guide to Alluvial Diagrams in ClinicoPath}
...
> Run `install.packages("survival")` and use `survival::colon` if you want to
> reproduce this example. The chunk is therefore not evaluated.

```{r survival-flow, eval = FALSE}
# Using the colon cancer dataset from the survival package
...
| `fillGgalluvial` | Variable | - | `NULL` | Fill variable, `ggalluvial` engine only |
| `maxvars` | Integer | - | `8` | Maximum number of variables allowed |
| `custombinlabels` | String | - | `""` | Comma-separated custom bin labels |
| `colorPalette` | List | `default`, `viridis`, `plasma`, `set3`, `pastel1`, `dark2` | `default` | Colour palette |
| `showCounts` | Boolean | `TRUE`/`FALSE` | `FALSE` | Show counts on nodes |
| `themeStyle` | List | `default`, `minimal`, `classic`, `grey`, `bw` | `default` | ggplot2 theme |
| `enhancedGradients` | Boolean | `TRUE`/`FALSE` | `FALSE` | Enhanced edge gradients |
| `plotSubtitle` | String | - | `""` | Plot subtitle |
| `weight` | Variable | - | `NULL` | Weight (frequency) variable |
| `sankeyStyle` | Boolean | `TRUE`/`FALSE` | `FALSE` | Sankey-style rendering |
| `curveType` | List | `cubic`, `linear`, `quintic`, `sine`, `arctangent`, `sigmoid` | `cubic` | Flow curve shape |
| `flowDirection` | List | `left_right`, `top_bottom`, `right_left`, `bottom_top` | `left_right` | Flow direction |
| `engine` | List | `easyalluvial`, `ggalluvial` | `easyalluvial` | Plotting engine |
| `labelNodes` | Boolean | `TRUE`/`FALSE` | `FALSE` | Label the nodes |
