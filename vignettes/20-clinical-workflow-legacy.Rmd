...
`vartree` has no defaults for `vars`, `percvar`, `summaryvar`, `prunebelow` and
`follow`, nor for the `Level` options `percvarLevel`, `pruneLevel1`,
`pruneLevel2`, `followLevel1` and `followLevel2` - the jamovi compiler forbids
defaults on `Level`. All of them are therefore required arguments of the R
wrapper and must be passed explicitly, using `NULL` where the feature is unused.
...
```{r}
# Age and sex distribution of the whole cohort.
# `female` and `male` are Level options and therefore REQUIRED arguments of the
# R wrapper - they name the levels of `gender` to place on each side.
# `agepyramid` has no grouping option, so a per-group pyramid means calling it
# once per subset.
agepyramid(
  data = clinical_data,
  age = "Age",
  ...
  female = "Female",
  male = "Male",
  plot_title = "Age and Sex Distribution"
)
```
...
# `summarydata` takes only continuous `vars`; it has no grouping option, so
# stratified summaries are produced by calling it on each subset.
...
### Biomarker Distribution Within a Treatment Group

```{r}
# Stratification is done by subsetting the data before the call
summarydata(
  data = dplyr::filter(clinical_data, Group == "Treatment"),
...
# factors rather than recoding to 0/1 numerics.
# `var1true` ... `var7true` are Level options and therefore REQUIRED arguments of
# the R wrapper: pass the positive level, or NULL when the variable is unused.
venn(
  data = clinical_data,
  var1 = "LVI",
  var1true = "Present",
...
# `benford` takes a single continuous variable; `digits` (default 2) controls
# whether first or first-two digits are analysed.
benford(
  data = clinical_data,
  var = "MeasurementA",
  digits = 2
...
statistics - `agreement()` and `icccoeff()` are provided by the **meddecide**
module. `reportcat()` below only tabulates the frequency distribution of each
rater's categorical scores.
